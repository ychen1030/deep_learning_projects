...
class QLearner(object):
    ...
    def __init__(self,
        ...
        q_target = q_func(obs_tp1_float, self.num_actions, scope='q_target_func', reuse=False)
        
        q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
        target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_target_func')
        
        if double_q:
            q_double_tmp = q_func(obs_tp1_float, self.num_actions, scope='q_func', reuse=True)
            act_double = tf.argmax(q_double_tmp, axis=1)
            q_tar_act = tf.reduce_sum(q_target * tf.one_hot(act_double, self.num_actions), axis=1)
        else:
            q_tar_act = tf.reduce_max(q_target, axis=1)
        
        self.best_action = tf.argmax(q, axis=1)
        
        y = self.rew_t_ph + (1.0 - self.done_mask_ph) * gamma * q_tar_act
        one_hot = tf.one_hot(self.act_t_ph, self.num_actions)
        diff = y - tf.reduce_sum(q * one_hot, axis=1)
        self.total_error = tf.reduce_mean(huber_loss(diff))
        # ----------------------------------------------------------------------
        # END OF YOUR CODE
        # ----------------------------------------------------------------------
