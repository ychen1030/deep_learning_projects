...
class Agent(object):
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # START OF YOUR CODE
            # ------------------------------------------------------------------
            sy_sampled_ac = tf.exp(sy_logstd) * tf.random_normal([self.ac_dim]) + sy_mean
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            for reward in re_n:
                results = np.zeros(len(reward))
                total = 0
                for i in reversed(range(len(reward))):
                    total = total * self.gamma + reward[i]
                    results[i] = total
                q_n.extend(results)
        else:
            for reward in re_n:
                ...
                for i in range(len(reward)):
                    total += np.power(self.gamma, i) * reward[i]
                results = np.ones(shape=[len(reward)]) * total
                q_n.extend(results)
        # ------------------------------------------------------------------
        # END OF YOUR CODE
        # ------------------------------------------------------------------
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        feed_dict = {self.sy_ob_no: ob_no, 
                     self.sy_ac_na: ac_na, 
                     self.sy_adv_n: adv_n}
        before = self.sess.run(self.loss, feed_dict = feed_dict)
        self.sess.run(self.update_op, feed_dict = feed_dict)
        after = self.sess.run(self.loss, feed_dict = feed_dict)
        # ------------------------------------------------------------------
        # END OF YOUR CODE
        # ------------------------------------------------------------------
