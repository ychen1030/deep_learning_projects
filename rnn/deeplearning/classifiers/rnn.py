...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # backward
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dx, cache_af)
    ...
    def sample(self, features, max_length=30):
        ...
        captions[:, 0] = self._start * np.ones((1, N), dtype=np.int32)
        
        for i in range(max_length):
            curr_embed = W_embed[captions[:, i]]
            if self.cell_type == 'rnn':
                prev_h, _ = rnn_step_forward(curr_embed, prev_h, Wx, Wh, b)
            else:
                prev_h, prev_c, _ = lstm_step_forward(curr_embed, prev_h, prev_c, Wx, Wh, b)
