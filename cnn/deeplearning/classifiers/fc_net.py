...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    ar, ar_cache = affine_relu_forward(X, self.params['W1'], self.params['b1'])
    scores, af_cache = affine_forward(ar, self.params['W2'], self.params['b2'])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dx, grads['W2'], grads['b2'] = affine_backward(dx, af_cache)
    _, grads['W1'], grads['b1'] = affine_relu_backward(dx, ar_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    """
    Initialize a new FullyConnectedNet.r
    ...
    for i in range(len(hidden_dims)):
      ...
      if i == 0:
        ...
        self.params['W' + sub] = weight_scale * np.random.randn(hidden_dims[i - 1], hidden_dims[i])
      self.params['b' + sub] = np.zeros(hidden_dims[i])

      if self.use_batchnorm:
        self.params['gamma' + sub] = np.ones(hidden_dims[i])
        self.params['beta' + sub] = np.zeros(hidden_dims[i])
  ...
  def loss(self, X, y=None):
    ...
    loss += 0.5*self.reg*(np.sum(self.params[last_w]**2))
    ...
    for i in range(self.num_layers - 1, 0, -1):
      ...
      if self.use_batchnorm:
        da, grads['gamma'+str(i)], grads['beta'+str(i)] = batchnorm_backward(da, norm_cache)
